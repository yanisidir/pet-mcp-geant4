...
void AnalyzeDetectedElectronCreationSeparation(
  const char* fileName = "build/mcp_output.root")
{
  ...
  if (!eventLeaf || !parentGammaTrackLeaf ||
      ...
      !creationTimeLeaf || !validCreationLeaf || !creatorProcessLeaf) {
    std::cerr
      << "ElectronChannelHitTree does not contain the creation "
      << "branches required for the exact association."
      << std::endl;
  ...
  // Determine the coordinate scale based on the branch name suffix
  ...
  // Loop over all detected electrons and group them by (eventID, parentGammaTrackID)
  for (Long64_t entry = 0; entry < electronEntries; ++entry) {
    ...
    // Only consider electrons with a known direct parent gamma track
  ...
  std::cout << "\nCreation-position association summary" << std::endl;
  ...
  std::cout << "  Successive electron-creation pairs: "
  ...
  if (absDzMm.empty()) {
    std::cerr << "\nNo gamma parent has at least two detected "
  ...
  std::cout << "\nDetected-electron creation separation statistics"
            << std::endl;
  std::cout
    << std::setw(12) << "Quantity"
    << " | " << std::setw(10) << "Mean [mm]"
  ...
  gStyle->SetOptStat(0);
  DrawHistogram(absDzMm,
                "detectedElectronCreationDz",
                "Longitudinal separation between detected-electron origins",
                "|#Deltaz| [mm]",
                "Fig/current/detected_electron_creation_dz.png",
                kBlue + 1);
  DrawHistogram(rxyMm,
                "detectedElectronCreationRxy",
                "Transverse separation between detected-electron origins",
                "r_{xy} [mm]",
                "Fig/current/detected_electron_creation_rxy.png",
                kRed + 1);
  DrawHistogram(distance3dMm,
                "detectedElectronCreationDistance3D",
                "3D separation between detected-electron origins",
                "3D distance [mm]",
                "Fig/current/detected_electron_creation_distance3d.png",
                kGreen + 2);

  std::cout << "\nSaved:" << std::endl;
  std::cout << "  detected_electron_creation_dz.png" << std::endl;
  std::cout << "  detected_electron_creation_rxy.png" << std::endl;
  std::cout << "  detected_electron_creation_distance3d.png"
            << std::endl;
